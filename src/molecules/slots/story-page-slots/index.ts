import { TopSlot } from "./top-slot";
import { BottomSlot } from "./bottom-slot";
import { LiveBlogCardSlot } from "./live-blog-card";
import { DefaultStoryCardSlot } from "./default-story-card";
import { RelatedStoryCardSlot } from "./related-story-card";
import { DefaultStoryElementSlot } from "./default-story-element-slot";

export { TopSlot, BottomSlot, LiveBlogCardSlot, DefaultStoryCardSlot, RelatedStoryCardSlot, DefaultStoryElementSlot };
